...
{
    float cameraSpeed = 40.5f * deltaT;
    ...
    if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS)
        freeCamPos += cameraSpeed * freeCamUp;
    if (glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS)
        freeCamPos -= cameraSpeed * freeCamUp;

    // Control Camera Pitch and Yaw with Arrow Keys
    ...
    vec3 front;
    front.x = cos(glm::radians(freeCamYaw)) * cos(glm::radians(freeCamPitch));
    front.y = sin(glm::radians(freeCamPitch));
    front.z = sin(glm::radians(freeCamYaw)) * cos(glm::radians(freeCamPitch));
...
void SceneBasic_Uniform::render()
{
    ...
    //Comment out to remove light frustum
